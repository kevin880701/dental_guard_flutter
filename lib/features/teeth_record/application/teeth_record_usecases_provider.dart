...
import '../domain/usecase/get_group_top_users_usecase.dart';
...
final createBrushingRecordUseCaseProvider =
    Provider<CreateBrushingRecordUseCase>((ref) {
  final repo = ref.read(teethRecordRepositoryProvider);
  return CreateBrushingRecordUseCase(repo);
});

final deleteBrushingRecordUseCaseProvider =
    Provider<DeleteBrushingRecordUseCase>((ref) {
  final repo = ref.read(teethRecordRepositoryProvider);
  return DeleteBrushingRecordUseCase(repo);
});

final getBrushingRecordByIdUseCaseProvider =
    Provider<GetBrushingRecordByIdUseCase>((ref) {
  final repo = ref.read(teethRecordRepositoryProvider);
  return GetBrushingRecordByIdUseCase(repo);
});

final getGroupsBrushingRecordsUseCaseProvider =
    Provider<GetGroupsBrushingRecordsUseCase>((ref) {
  final repo = ref.read(teethRecordRepositoryProvider);
  return GetGroupsBrushingRecordsUseCase(repo);
});

final getGroupBrushingStatsUseCaseProvider =
    Provider<GetGroupBrushingStatsUseCase>((ref) {
  final repo = ref.read(teethRecordRepositoryProvider);
  return GetGroupBrushingStatsUseCase(repo);
});

final getUserBrushingStatsUseCaseProvider =
    Provider<GetUserBrushingStatsUseCase>((ref) {
  final repo = ref.read(teethRecordRepositoryProvider);
  return GetUserBrushingStatsUseCase(repo);
});

final getUsersRecordsSearchUseCaseProvider =
    Provider<UsersRecordsSearchUseCase>((ref) {
  final repo = ref.read(teethRecordRepositoryProvider);
  return UsersRecordsSearchUseCase(repo);
});

final getGroupsTopUsersUseCaseProvider =
    Provider<GetGroupsTopUsersUseCase>((ref) {
  final repo = ref.read(teethRecordRepositoryProvider);
  return GetGroupsTopUsersUseCase(repo);
});
...
final groupRankProvider = FutureProvider.autoDispose.family<
    List<GroupTopUser>,
    ({
      String groupId,
      DateTime selectTime,
      ChartTimeStatus status,
      RankSortCriteria sortBy,
      int limit,
      int refreshKey,
    })>((ref, param) async {
  final useCase = ref.read(getGroupsTopUsersUseCaseProvider);

  // 根據傳入的時間和狀態計算日期範圍
  final dateRange = getChartDateRange(param.selectTime, param.status);
  final String timeZone = await FlutterTimezone.getLocalTimezone();

  // 呼叫 use case 並傳入 API 請求所需的參數
  final result = await useCase(
    groupId: param.groupId,
    sortCriteria: param.sortBy,
    // 將 sortBy 傳遞給 sortCriteria
    limit: param.limit,
    startDate: dateRange.start,
    endDate: dateRange.end,
    timeZone: timeZone,
